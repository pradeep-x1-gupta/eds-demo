...
/**
 * Transformer: kaiserpermanente section breaks + section metadata.
 * Section selectors come from page-templates.json (DOM-verified boundaries).
 * Follows the reference implementation: insert <hr> breaks in beforeTransform
 * (while every section element still exists), then anchor Section Metadata
 * blocks in afterTransform via a temporary marker.
 */
...
export default function transform(hookName, element, payload) {
  ...
  if (hookName === 'afterTransform') {
    ...
    // the marker <hr>, or (first section, no marker) the original element.
    for (let i = sections.length - 1; i >= 0; i -= 1) {
      const section = sections[i];
